...
public class Extractor {
    ...
    public static Optional<String> sessionIdFrom(String cookieLine, String sessionKey) {
        String[] cookies = cookieLine.split(DELIMITER_BLANK);
        return Arrays.stream(cookies)
                .filter(cookie -> isSessionId(cookie, sessionKey))
                .map(line -> line.split(DELIMITER_EQUAL)[1])
                .findAny();
    }

    private static boolean isSessionId(String cookie, String sessionKey) {
        return cookie.split(DELIMITER_EQUAL)[0].equals(sessionKey);
    }
}
