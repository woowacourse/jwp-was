...
class HttpHeaderTest {
    ...
    void 생성(String key, String value) {
        HttpHeader httpHeader = HttpHeader.of(Arrays.asList(key + ": " + value));
    ...
    void 형식에_맞지_않는_헤더_라인(String value) {
        HttpHeader httpHeader = HttpHeader.of(Arrays.asList(value));
    ...
    void 헤더에_빈_칸이_들어가는_경우() {
        HttpHeader httpHeader = HttpHeader.of(Arrays.asList(""));
    ...
    @Test
    void 헤더_사이즈가_0인_경우() {
        assertEquals(HttpHeader.of(Arrays.asList()).getHeaders(), Collections.emptyMap());
    }
}