...
class HttpRequestAssemblerTest {
    ...
    private static final String REQUEST_GET_LINE_FORMAT =
        HttpMethod.GET.name() + " %s " + Constants.HTTP_VERSION + lineSeparator();
    private static final String REQUEST_POST_LINE_FORMAT =
        HttpMethod.POST.name() + " %s " + Constants.HTTP_VERSION + lineSeparator();
    ...
    void assemble_hasNotQueryString(String url) throws IOException {
        ...
        Assertions.assertThat(httpRequest.getUrlPath()).isEqualTo(url.split("\\?")[0]);
        Assertions.assertThat(httpRequest.getParameters()).isEmpty();
    }
    ...
    void assemble_hasQueryString(String url) throws IOException {
        ...
        Assertions.assertThat(httpRequest.getUrlPath()).isEqualTo(urlParameters[0]);

        int parameterCount = urlParameters[1].split("&").length;
        Assertions.assertThat(httpRequest.getParameters()).hasSize(parameterCount);
    }
    ...
    void assemble_hasBody(String body, int parameterCount) throws IOException {
        ...
        String request =
            String.format(REQUEST_POST_LINE_FORMAT, Constants.URL_PATH_INDEX_HTML)
                + contentLengthHeader
